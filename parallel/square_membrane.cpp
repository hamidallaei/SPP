...
void Run(Box& box, int argc, char *argv[])
{
	...
	if (box.thisnode->node_id == 0)
	{
		...
		cout << "2-number of beads on each membrane side" << endl;
	...
	Real input_membrane_side_bead = atof(argv[2+input_file]);
	Real input_chiral_radius = atof(argv[3+input_file]);
	Real input_packing_fraction = atof(argv[4+input_file]);

	int input_Nm = (int) 4*input_membrane_side_bead - 4;
//	int input_Ns = (int) round(input_packing_fraction*input_Nm*input_Nm / (input_chain_length*M_PI*M_PI));
	int input_Ns = (int) round(input_packing_fraction*input_membrane_side_bead*input_membrane_side_bead / input_chain_length);
...
// The following must be before box.init
	for (int i = 0; i < input_Nm; i++)
	{
		...
		box.particle[i].m_parallel = 0;
		box.particle[i].m_perpendicular = 0;
	}
	...
	box.membrane_radius = input_membrane_side_bead / 2;
...
//			Ring_Membrane(box.particle, box.Nm);
			Square_Membrane(box.particle, input_membrane_side_bead);
//			Confined_In_Ring_Membrane(box.particle, box.Ns, box.Nm);
			Confined_In_Square_Membrane(box.particle, box.Ns, input_membrane_side_bead);
		}
		box.Sync();
	}
